...
def go_page(delta):
    st.session_state.pg = max(0, min(len(PAGES)-1, st.session_state.pg + delta))
    st.rerun()
...
def build_pdf(t, score, sb, ins, warn, dng):
    ...
    kv([
        ...
        ("SEBI Registration No.", ADVISOR["reg_no"]),
    ])
    ...
    story.append(Paragraph(
        ...
        f"Consult a SEBI-registered investment advisor before making any financial decision. "
        f"Report generated by FinSight on {now}. | {ADVISOR['name']} | SEBI Reg. No. {ADVISOR['reg_no']}",
        SM))
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    <div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:12px;padding:18px 24px;margin-top:8px">
      <p style="font-size:.77rem;color:#64748b;margin:0;line-height:1.7">
        ...
        Please consult a SEBI-registered investment advisor before making any financial decision.
        <br><b>{ADVISOR['name']}</b> | {ADVISOR['title']} | SEBI Reg. No. {ADVISOR['reg_no']}
      </p>
    </div>""", unsafe_allow_html=True)
